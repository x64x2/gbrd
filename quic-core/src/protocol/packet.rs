...
impl Packet {
	...
	pub const fn new(assoc_id: u16, pkt_id: u16, frag_total: u8, frag_id: u8, size: u16, addr: Address) -> Self {
		Self {
			assoc_id,
			pkt_id,
			frag_total,
...
impl From<Packet> for (u16, u16, u8, u8, u16, Address) {
	fn from(pkt: Packet) -> Self {
		(pkt.assoc_id, pkt.pkt_id, pkt.frag_total, pkt.frag_id, pkt.size, pkt.addr)
	}
}
