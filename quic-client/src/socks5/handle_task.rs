...
use quic_core::Address as quicAddress;

use super::{Server, UDP_SESSIONS, udp_session::UdpSession};
use crate::connection::{Connection as quicConnection, ERROR_CODE};
...
impl Server {
	pub async fn handle_associate(
		...
		match UdpSession::new(assoc_id, peer_addr, local_ip, dual_stack, max_pkt_size) {
			Ok(session) => {
					...
					loop {
						...
						let forward = async move {
							let target_addr = match target_addr {
								Address::DomainAddress(domain, port) => quicAddress::DomainAddress(domain, port),
								Address::SocketAddress(addr) => quicAddress::SocketAddress(addr),
							};

							match quicConnection::get_conn().await {
				...
				if let Ok(conn) = quicConnection::get_conn().await
	...
	pub async fn handle_connect(conn: Connect<connect::NeedReply>, addr: Address) {
		...
		let relay = match quicConnection::get_conn().await {
