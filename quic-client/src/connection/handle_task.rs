...
impl Connection {
	pub async fn authenticate(self, zero_rtt_accepted: Option<ZeroRttAccepted>) {
		...
		match self.model.authenticate(self.uuid, self.password.clone()).await {
		...
		match self.udp_relay_mode {
			UdpRelayMode::Native => {
				...
				match self.model.packet_native(pkt, addr, assoc_id) {
					Ok(()) => Ok(()),
					Err(err) => {
						warn!("[relay] [packet] [{assoc_id:#06x}] [to-native] to {addr_display}: {err}");
						Err(err)?
					}
				}
			}
			UdpRelayMode::Quic => {
				...
				match self.model.packet_quic(pkt, addr, assoc_id).await {
					Ok(()) => Ok(()),
					Err(err) => {
						warn!("[relay] [packet] [{assoc_id:#06x}] [to-quic] to {addr_display}: {err}");
						Err(err)
					}
				}
			}
	...
	pub async fn handle_packet(pkt: Packet) {
		...
		match pkt.accept().await {
			Ok(Some((pkt, addr, _))) => {
				info!("[relay] [packet] [{assoc_id:#06x}] [from-{mode}] [{pkt_id:#06x}] from {addr}");

				let addr = match addr {
					Address::None => unreachable!(),
					Address::DomainAddress(domain, port) => Socks5Address::DomainAddress(domain, port),
				...
				if let Some(session) = session {
					if let Err(err) = session.send(pkt, addr).await {
						warn!(
							"[relay] [packet] [{assoc_id:#06x}] [from-native] [{pkt_id:#06x}] failed sending packet to socks5 \
							 client: {err}",
						);
					}
				} else {
					...
					if let Some(session) = fwd_session {
						if let Err(err) = session.send(pkt).await {
							warn!(
								"[relay] [packet] [{assoc_id:#06x}] [from-native] [{pkt_id:#06x}] failed sending packet to \
								 UDP forwarder client: {err}",
							);
						}
					} else {
						warn!("[relay] [packet] [{assoc_id:#06x}] [from-native] [{pkt_id:#06x}] unable to find UDP session");
					}
				}
			}
			Ok(None) => {}
			Err(err) => warn!("[relay] [packet] [{assoc_id:#06x}] [from-native] [{pkt_id:#06x}] packet receiving error: {err}"),
		}
	}
}
