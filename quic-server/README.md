...
Minimalistic quic server implementation as a reference.
...
`quic-server` is a robust and actively developed implementation of the quic protocol server. It is a fork of the original quic project with significant enhancements and additional features. While it started as a reference implementation, it has evolved to include many production-ready capabilities.
...
- Minimal quic protocol server implementation
...
Get the latest release from [GitHub Releases](https://github.com/Itsusinn/quic/releases).

### Install via Cargo

```bash
cargo install --git https://github.com/Itsusinn/quic.git quic-server
```
...
Run the quic server with a configuration file:

```bash
# Specify config file directly
quic-server -c PATH/TO/CONFIG

# Specify config directory (automatically finds first config file)
quic-server -d PATH/TO/CONFIG_DIR
...
```bash
docker run --name quic-server \
  --restart always \
  --network host \
  -v /PATH/TO/CONFIG_FILE:/etc/quic/config.toml \
  -v /PATH/TO/CERTIFICATE:/PATH/TO/CERTIFICATE \
  -v /PATH/TO/PRIVATE_KEY:/PATH/TO/PRIVATE_KEY \
  -dit ghcr.io/itsusinn/quic-server:latest
  ## or -dit docker.io/itsusinn/quic-server:latest
```

**Note:** The Docker image now uses `-d /etc/quic` by default, allowing you to mount your config directory.
...
services:
  quic:
    image: ghcr.io/itsusinn/quic-server:latest
    ## or image: docker.io/itsusinn/quic-server:latest
    restart: always
    container_name: quic
    network_mode: host
    volumes:
      - ./config.toml:/etc/quic/config.toml:ro  # Mount config file
      #- ./config:/etc/quic:ro # Mount config directory
      - ./cert.crt:/PATH/TO/CERT:ro
      - ./key.crt:/PATH/TO/KEY:ro
```

The server will automatically detect and use the first config file found in `/etc/quic`.
...
The format is automatically detected based on the file extension. You can also force TOML parsing by setting the `quic_FORCE_TOML` environment variable. Or use `quic_CONFIG_FORMAT` environment variable to explicitly specify the format (`toml` or `json5`).
...
# Working directory for quic-server (used for relative certificate/key paths)
data_dir = ""
...
`quic-server` includes built-in ACME support for automatic SSL certificate provisioning via Let's Encrypt. This allows the server to automatically obtain and renew certificates without external tools.
...
- If running as a non-root user on Linux, you may need to allow binding to privileged ports:
  ```sh
  setcap CAP_NET_BIND_SERVICE=+eip <path to quic-server binary>
  ```
- If ACME provisioning fails, quic-server will fall back to self-signed certificates if configured.
...
Contributions, bug reports, and feature requests are welcome! Please open issues or pull requests on [GitHub](https://github.com/Itsusinn/quic).
